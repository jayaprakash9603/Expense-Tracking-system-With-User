...
const getInitialState = () => {
  ...
  if (jwt) {
    ...
    const savedPalette = localStorage.getItem(STORAGE_KEYS.THEME_PALETTE);
    const savedUseSystem = localStorage.getItem(STORAGE_KEYS.THEME_USE_SYSTEM);

    const useSystem = savedUseSystem === "true";
    const mode = useSystem ? getSystemPreference() : savedMode || DEFAULT_THEME_MODE;
    const palette =
      savedPalette && COLOR_PALETTES[savedPalette] ? savedPalette : DEFAULT_PALETTE;

    return { mode, palette, useSystemPreference: useSystem };
  }
...
const persistTheme = (mode, palette, useSystem) => {
  ...
  localStorage.setItem(STORAGE_KEYS.THEME_USE_SYSTEM, String(useSystem));
};
...
export const themeReducer = (state = initialState, action) => {
  switch (action.type) {
    case TOGGLE_THEME: {
      ...
      return { ...state, mode: newMode, useSystemPreference: false };
    }
    ...
    case SET_THEME_FULL: {
      ...
      return { ...state, mode: newMode, palette: newPalette, useSystemPreference: false };
    }
    ...
    case SET_SYSTEM_PREFERENCE: {
      ...
      return { ...state, mode: newMode, useSystemPreference: useSystem };
    }

    case RESET_THEME: {
      persistTheme(DEFAULT_THEME_MODE, DEFAULT_PALETTE, false);
      return { mode: DEFAULT_THEME_MODE, palette: DEFAULT_PALETTE, useSystemPreference: false };
    }
