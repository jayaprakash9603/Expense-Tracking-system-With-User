...
export function CategoryReportCharts({ category, categoryRefreshing = false }) {
  const { t } = useLanguage();
  ...
  return (
    <ResponsiveGrid cols={{ default: 1, md: 2 }} gap="lg">
      <ChartCard
        title={t("reports.categoryAmounts")}
        className={cn("transition-opacity", categoryRefreshing && "opacity-60")}
      >
