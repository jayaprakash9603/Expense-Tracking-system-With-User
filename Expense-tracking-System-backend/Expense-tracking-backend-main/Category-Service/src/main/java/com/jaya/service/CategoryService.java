...
public class CategoryService {
    ...
    public Category create(Category category, Integer userId) throws Exception {
        ...
        // Return immediately with the created category; expense mappings will be
    ...
    public Category getById(Integer id, Integer userId) throws Exception {
        ...
        if (category.isGlobal()) {
            ...
            // User can access global category if they haven't "deleted" it and haven't
            // edited it
    ...
    public Category update(Integer id, Category category, Integer userId) throws Exception {
        ...
        if (existing.isGlobal() && !isUserEdited(userId, id)) {
            ...
            // Exclusivity: remove requested from all other categories (exclude source and
            // the new one)
        ...
        // Enforce exclusivity: remove newIds from all other categories (exclude
        // current)
    ...
    private void updateExpenseEntitiesCategory(Integer userId, Set<Integer> expenseIds, Integer targetCategoryId,
            String targetCategoryName) throws Exception {
    ...
    public List<Category> updateMultiple(List<Category> categories, Integer userId) throws Exception {
        ...
        // Step 0: Collect all previous expense IDs for each category and build a map of
        // all expenses
        ...
        // Step 1: Build mapping of expenseId -> categories that want to claim it in the
        // new update
        ...
        // Step 2: Find duplicate expense IDs (assigned to more than one category in the
        // update)
        ...
        for (Map.Entry<Integer, Integer> entry : previousExpenseToCategoryMap.entrySet()) {
            ...
            // If this expense is not in any new category assignments and not already marked
            // as moved
        ...
        for (Category category : categories) {
            ...
            for (Map.Entry<Integer, Integer> entry : movedExpenses.entrySet()) {
                ...
                // If this expense has been moved to a different category, remove it from all
                // others
        ...
        // Step 6: Find or create "Others" category for duplicates and completely
        // removed expenses
        ...
        for (Category category : categories) {
            ...
            if (existing != null) {
                ...
                // Get the expense IDs for this user from the input category (already filtered
                // for duplicates)
    ...
    public void deleteAllGlobal(Integer userId, boolean global) throws Exception {
        ...
        if (global) {
            // Handle global categories deletion - using optimized query with EntityGraph
     ...
     * Optimized to prevent N+1 query problem using EntityGraph
     */
    public List<Category> getAllForUser(Integer userId) {
        // Get user's own categories with all collections eagerly loaded
        List<Category> userCategories = categoryRepository.findByUserIdWithDetails(userId);

        // Get global categories with all collections eagerly loaded
     ...
     * @return A list of all user expenses with category flag, ordered by category
     *         membership
     */
    public List<ExpenseDTO> getAllUserExpensesOrderedByCategoryFlag(
        ...
        // Separate expenses into two lists: those in the category and those not in the
        // category
        ...
        // Combine the lists: first those in the category, then those not in the
        // category
