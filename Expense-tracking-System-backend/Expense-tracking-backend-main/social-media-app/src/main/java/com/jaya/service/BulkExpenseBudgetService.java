...
/**
 * Service for handling bulk expense and budget creation with linking and recovery
 */
@Service
@Slf4j
public class BulkExpenseBudgetService {
    ...
    private void processMappingGroup(
        ...
        // Step 1: Publish budget creation events FIRST (so Budget Service creates them before expenses need them)
        if (mapping.getBudgets() != null && !mapping.getBudgets().isEmpty()) {
            for (BulkExpenseBudgetRequest.BudgetData budgetData : mapping.getBudgets()) {
                try {
                    ...
                    // Note: We haven't created expenses yet, so we pass the current oldToNewExpenseIds mapping
            ...
            if (expenseData.getBudgetIds() != null) {
                for (Long oldBudgetId : expenseData.getBudgetIds()) {
                    budgetMappings.add(BulkExpenseBudgetResponse.BudgetMapping.builder()
                        ...
                        .newBudgetId(oldToNewBudgetIds.get(oldBudgetId)) // Will be null until Kafka event updates it
                        .success(true)
                        .build());
                }
            }
    ...
    private Expense createExpenseFromData(BulkExpenseBudgetRequest.ExpenseData data, Integer userId) throws Exception {
        ...
        details.setPaymentMethod(expenseDetails.getPaymentMethod() != null ? expenseDetails.getPaymentMethod() : "cash");
        details.setNetAmount(expenseDetails.getNetAmount() != null ? expenseDetails.getNetAmount() : -expenseDetails.getAmount());
    ...
    private void publishBudgetCreationEvent(
        ...
        if (budgetData.getExpenseIds() != null && !budgetData.getExpenseIds().isEmpty()) {
            for (Long oldId : budgetData.getExpenseIds()) {
                ...
                if (newId != null) {
                    ...
                    log.debug("Expense old ID {} not yet mapped, will be linked via EXPENSE_CREATED_WITH_OLD_BUDGETS event", oldId);
                }
            }
        }
        ...
        ExpenseBudgetLinkingEvent.BudgetDetails budgetDetails = ExpenseBudgetLinkingEvent.BudgetDetails.builder()
            ...
            .remainingAmount(budgetData.getRemainingAmount() != null ? budgetData.getRemainingAmount() : budgetData.getAmount())
        ...
        try {
            ...
            log.info("Published budget creation event for old budget ID: {} with {} mapped expense IDs (out of {} total)", 
    ...
    /**
     * Publish event when expense is created with existing budgets (replace old expense ID with new)
     */
    private void publishExpenseCreatedWithExistingBudgetsEvent(
        ...
        publishAfterCommit(event);
        log.info("Published EXPENSE_CREATED_WITH_EXISTING_BUDGETS event for new expense {} (old: {}) with {} existing budgets",
