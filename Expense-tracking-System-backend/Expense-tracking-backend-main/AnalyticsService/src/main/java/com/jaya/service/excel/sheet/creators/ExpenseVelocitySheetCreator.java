...
/**
 * Creates the Expense Velocity sheet showing spending momentum and projections.
 */
@Component
public class ExpenseVelocitySheetCreator extends AbstractSheetCreator {
    ...
    private int createRecentActivitySection(XSSFSheet sheet, int rowIdx, ExpenseVelocityData velocity, ExcelStyleFactory sf) {
    ...
    private int createProjectionsSection(XSSFSheet sheet, int rowIdx, ExpenseVelocityData velocity, ExcelStyleFactory sf) {
        rowIdx = createSectionHeader(sheet, rowIdx, "Projections & Trends", sf, 2);
        
        XSSFCellStyle labelStyle = sf.createKpiLabelStyle();
        XSSFCellStyle valueStyle = sf.createKpiCurrencyStyle();
        
        // Trend row with conditional styling
        ...
        rowIdx = addKpiRow(sheet, rowIdx, "Projected Monthly Spend", velocity.getProjectedMonthlySpend(), labelStyle, valueStyle);
    ...
    private XSSFCellStyle getTrendStyle(String trend, ExcelStyleFactory sf) {
        switch (trend) {
            case "INCREASING": return sf.createWarningStyle();
            case "DECREASING": return sf.createSuccessStyle();
            default: return sf.createDataStyle();
        }
    }
}
