...
public interface ChatRepository extends JpaRepository<Chat, Integer> {
    ...
    // Paginated conversation between users
    @Query("SELECT c FROM Chat c WHERE " +
...
List<Chat> findByGroupIdNotDeletedByUser(@Param("groupId") Integer groupId, @Param("userId") Integer userId);
    ...
    // Search methods with deletion filtering
    @Query("SELECT c FROM Chat c WHERE c.recipientId = :recipientId AND " +
    ...
    List<Chat> findByGroupIdAndContentContainingIgnoreCaseNotDeleted(
            @Param("groupId") Integer groupId, @Param("keyword") String keyword, @Param("userId") Integer userId);

    // Unread messages with deletion filtering
    ...
    @Query("SELECT c FROM Chat c WHERE c.groupId = :groupId AND " +
            ...
            "c.senderId != :userId")
    List<Chat> findByGroupIdAndNotReadByUserAndNotDeleted(@Param("groupId") Integer groupId, @Param("userId") Integer userId);
    ...
    // Replace the problematic countActiveConversationsForUser method with this corrected version:
    ...
    // Helper method to get the most active chat partner
    ...
    // Replace the problematic findRecentChatsForUser method with this corrected version:
    @Query("SELECT c FROM Chat c WHERE " +
            "(c.senderId = :userId OR c.recipientId = :userId OR c.groupId IS NOT NULL) " +
            "ORDER BY c.timestamp DESC")
    ...
    // Replace the problematic searchAllUserMessagesAdvanced method:
    @Query("SELECT c FROM Chat c WHERE " +
    ...
    // Find by IDs method for archived chats
