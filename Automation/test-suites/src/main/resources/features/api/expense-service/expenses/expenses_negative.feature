@expense @api @negative @expenses
Feature: expense-service expenses API negative scenarios
  ...
  Scenario: Unauthorized request is rejected
    When the user sends a GET request to "expenses.create"
  ...
  Scenario: Authenticated validation failure on create
    ...
    Given request body "invalidBody" uses the "expense-create-invalid.json" payload
    When the user sends a POST request to "expenses.create" using request body "invalidBody"
    Then the response status should be one of "400,422,500"
    And the response should match the "expense-error" schema
