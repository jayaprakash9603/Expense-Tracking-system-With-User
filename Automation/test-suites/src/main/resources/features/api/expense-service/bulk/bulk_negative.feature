@expense @api @negative @bulk
Feature: expense-service bulk API negative scenarios
  ...
  Scenario: Unauthorized request is rejected
    When the user sends a GET request to "bulk.expenses-budgets.create"
  ...
  Scenario: Unknown resource id returns not found
    Given the user is logged in with test credentials
    When the user sends a GET request to "bulk.expenses-budgets.progress" with data
      | key        | value   |
