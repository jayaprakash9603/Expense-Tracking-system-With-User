@bill @api @negative @bills
Feature: bill-service bills API negative scenarios
  ...
  Scenario: Unauthorized request is rejected
    When the user sends a GET request to "bills.create"
  ...
  Scenario: Authenticated validation failure on create
    ...
    Given request body "invalidBody" uses the "bill-create-invalid.json" payload
    When the user sends a POST request to "bills.create" using request body "invalidBody"
    Then the response status should be one of "400,422,500"
    And the response should match the "bill-error" schema
