@story @api @negative @stories
Feature: story-service stories API negative scenarios
  ...
  Scenario: Unauthorized request is rejected
    When the user sends a GET request to "stories.create"
  ...
  Scenario: Authenticated validation failure on create
    ...
    Given request body "invalidBody" uses the "story-create-invalid.json" payload
    When the user sends a POST request to "stories.create" using request body "invalidBody"
    Then the response status should be one of "400,422,500"
    And the response should match the "story-error" schema
