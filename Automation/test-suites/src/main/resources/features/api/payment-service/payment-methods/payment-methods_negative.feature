@payment @api @negative @payment-methods
Feature: payment-service payment-methods API negative scenarios
  ...
  Scenario: Unauthorized request is rejected
    When the user sends a GET request to "payments.create"
  ...
  Scenario: Authenticated validation failure on create
    ...
    Given request body "invalidBody" uses the "payment-create-invalid.json" payload
    When the user sends a POST request to "payments.create" using request body "invalidBody"
    Then the response status should be one of "400,422,500"
    And the response should match the "payment-error" schema
