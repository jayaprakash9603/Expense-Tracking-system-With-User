@audit @api @negative @audit
Feature: audit-service audit API negative scenarios
  ...
  Scenario: Authenticated validation failure on create
    ...
    Given request body "invalidBody" uses the "audit-report-invalid.json" payload
    When the user sends a POST request to "audit.admin-generate-report" using request body "invalidBody"
    Then the response status should be one of "400,422,500"
    And the response should match the "audit-error" schema

  @negative @notfound
  Scenario: Unknown resource id returns not found
    Given the user is logged in with test credentials
    When the user sends a GET request to "audit.admin-by-user" with data
      | key        | value   |
