@category @api @negative @categories
Feature: category-service categories API negative scenarios
  ...
  Scenario: Unauthorized request is rejected
    When the user sends a GET request to "categories.create"
  ...
  Scenario: Authenticated validation failure on create
    ...
    Given request body "invalidBody" uses the "category-create-invalid.json" payload
    When the user sends a POST request to "categories.create" using request body "invalidBody"
    Then the response status should be one of "400,422,500"
    And the response should match the "category-error" schema
